...
"""
                ...
                if answer_records:
                    for i, record in enumerate(answer_records, 1):
                        ...
                        elapsed = record.timestamp - (
                            answer_records[0].timestamp if answer_records else record.timestamp
                        )
    ...
    def log_system_state_snapshot(self, orchestrator, phase: str = "unknown"):
        ...
        system_snapshot = {
            ...
            "system_phase": phase,
            "system_runtime": (
                (time.time() - orchestrator.system_state.start_time) if orchestrator.system_state.start_time else 0
            ),
        }
