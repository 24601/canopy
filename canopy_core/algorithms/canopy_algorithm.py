...
# Based on the original MassGen framework: https://github.com/Leezekun/MassGen
"""
Canopy algorithm implementation.

This module implements the original consensus-based orchestration
...
class CanopyAlgorithm(BaseAlgorithm):
    """Canopy consensus-based orchestration algorithm.

    This algorithm implements the original consensus approach where:
    ...
    def _run_all_agents_with_dynamic_restart(self, task: TaskInput) -> None:
        ...
        try:
            ...
            self._start_initial_agents(task, executor, active_futures)

            # Monitor agents and handle restarts
            self._monitor_agents_loop(task, executor, active_futures)

        finally:
            self._cleanup_executor(executor, active_futures)

    def _start_initial_agents(self, task: TaskInput, executor, active_futures):
        """Start all initial working agents."""
    ...
    def _monitor_agents_loop(self, task: TaskInput, executor, active_futures):
        """Main monitoring loop for agent execution."""
        while active_futures and not self._all_agents_voted():
            self._process_completed_agents(active_futures)
            self._restart_working_agents(task, executor, active_futures)
            time.sleep(0.1)  # Small delay to prevent busy waiting

    def _process_completed_agents(self, active_futures):
        """Process completed agents and handle any exceptions."""
        completed_futures = []
    ...
    def _cleanup_executor(self, executor, active_futures):
        """Clean up executor and cancel remaining futures."""
...
register_algorithm("massgen", CanopyAlgorithm)
