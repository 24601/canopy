...
class MultiRegionDisplay:
    def __init__(
        ...
        self.algorithm_name: str = "massgen"  # Track which algorithm is being used
    ...
    def _update_display_immediate(self) -> None:
        ...
        if self.save_logs and (hasattr(self, "session_logs_dir") or self.answers_dir):
            ...
            for agent_id in agent_ids:
                ...
                if answer_path:
                    ...
                    if log_path:
                        display_path = (
                            log_path.replace(os.getcwd() + "/", "") if log_path.startswith(os.getcwd()) else log_path
                        )
                        prefix = "📁 Log: "
                        max_path_len = max(10, col_width - self._get_display_width(prefix) - 8)
                        if len(display_path) > max_path_len:
        ...
        if self.system_messages or self.current_phase or self.vote_distribution:
            ...
            system_state_info.append(
                f"{BRIGHT_WHITE}Algorithm:{RESET} {BRIGHT_CYAN}{self.algorithm_name.upper()}{RESET}"
            )
